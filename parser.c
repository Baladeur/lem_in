...
/*   Updated: 2019/12/04 16:23:52 by myener           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ant	*ant_parser(t_ant *ant, int ant_nb)
{
	int		i;
	t_ant	*tmp;
	t_ant	*head;

	ant = ant_malloc(ant); // malloc first node.
	tmp = ant;
	head = ant;
	ant->id = 1; // give id to first node.
	ant->position = 0; // by default all the ants begin in the start room, aka id 0.
	i = 2;
	while (i <= ant_nb)
	{
		ant->next = ant_malloc(ant->next);
		ant = ant->next;
		ant->prev = tmp;
		tmp = ant;
		ant->id = i;
		ant->position = 0; // default
		i++;
	}
	ant->next = NULL;
	return (head);
...
void	room_parser_stocker(char *line, t_room *room) // stocks individual room nodes with data.
{
	int		i;
	int		start;
...
t_room	*room_parser(char *line, t_room *head) // parses every classic room data.
{
	...
	room = head; // first we keep track of the head, as we will need to return it.
	if (!room) // if this is the first node, initialize accordingly.
	{
		room = room_malloc(room); // malloc and initializes the new node.
		room->prev = NULL; // initializes the first node's prev, aka NULL.
		room->next = NULL;// initializes the first node's next, aka NULL.
	}
	else if (room && room->name) // else if there's already at least one node, fill the next one.
	{
		while (room && room->name && room->next) // goes to the last element of the list.
			room = room->next;
		tmp = room; // stocks the last known node, which will be NULL if no node is already present.
		room->next = room_malloc(room->next); // malloc and initializes the new node.
		room = room->next; // goes to the new node;
		room->prev = tmp; // initializes the new last node's prev, aka the old last node.
		room->next = NULL;// initializes the new last node's next, aka NULL.
	}
	room_parser_stocker(line, room);
	return (head);
}
...
t_ant    *lem_in_parser(char **data, t_room *room, t_ant *ant) // parses the given data to get various info.
{
    int     i;
	int		ret;
	int		ant_nb;
    ...
    while (data[i])
    {
		if (data[i][0] == 'L') // lines can't start with L. If they do, output error.
			parsing_error(data);
		if (data[i][0] == '#') // if it starts with #, it might be a comment or a command:
		{
		...
		else if ((data[i][0] >= 33 && data[i][0] <= 126)) // if it starts with a letter, number or special character, it might be a room or a path:
		{
			...
			room = (ret = (room_or_path(data[i])) == 1) ? room_parser(data[i], room) : room;
		}
		i++;
    }
	// room_add_start_end(data, room, &info);
	return (ant);
}
